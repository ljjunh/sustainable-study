  ...
  return (
    <section className="week-board">
      <div className="week-header">
        ...
        {days.map((day) => (
          ...
          >
            ...
            <strong className={day.isToday ? "is-today" : ""}>
              {day.dayOfMonth}
            </strong>
          </button>
        ))}
      ...
      <div className="week-all-day">
        ...
        {days.map((day) => {
          const dayTasks = getTasksForDate(tasks, day.date).filter(
            (task) => !task.done
          );
          return (
            <button
            ...
            >
              {dayTasks.slice(0, 2).map((task) => (
                <span
                  className={`week-task-chip ${priorityClass[task.priority]}`}
                  key={task.id}
                >
                  {task.title}
                </span>
              ...
              {dayTasks.length > 2 ? (
                <span className="week-more">+{dayTasks.length - 2}</span>
              ) : null}
            </button>
          );
        })}
              ...
              >
                ...
                {drag?.date === day.date ? (
                  <span
                    ...
                    style={rangeStyle(drag.start, drag.end)}
                  />
                ) : null}
                {dayEvents.map((event) => (
                  <button
      ...
      {editor ? (
        <EventEditorModal
          key={
            editor.mode === "edit"
              ? editor.eventId
              : `${editor.draft.date}-${editor.draft.startTime}-${editor.draft.endTime}`
          }
...
function eventStyle(event: ScheduleEvent): CSSProperties {
  ...
  const end = Math.max(
    ...
    clampMinutes(timeToMinutes(event.endTime))
  );
